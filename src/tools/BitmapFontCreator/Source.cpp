...
std::wstring const gNameSpace = L"forward";
LONG gWindowWidth = 1024;
LONG gWindowHeight = 256;
forward::FileSystem gFileSystem;
...
bool CreateHeaderFile(std::wstring const& fontName, int weight, int size, DWORD italic)
{
	...
	std::wstring fileFullPath = gFileSystem.GetFontFolder() + fileName;
	FILE* output = nullptr;
	errno_t result = _wfopen_s(&output, fileFullPath.c_str(), L"wt");
	if (result != 0 || !output)
	{
		return false;
	}
	...
	fwprintf(output, L"\n");
	fwprintf(output, L"#include \"Font.h\"\n");
	fwprintf(output, L"\n");
	...
	fwprintf(output, L"    %s(i32 maxMessageLength);\n", cn);
	fwprintf(output, L"\n");
	fwprintf(output, L"private:\n");
	fwprintf(output, L"    static i32 msWidth;\n");
	fwprintf(output, L"    static i32 msHeight;\n");
	fwprintf(output, L"    static u8 msTexels[];\n");
	fwprintf(output, L"    static f32 msCharacterData[];\n");
	fwprintf(output, L"};\n");
	fwprintf(output, L"\n");
	fwprintf(output, L"}\n");
	...
	float const* characterData)
{
	...
	std::wstring fileFullPath = gFileSystem.GetFontFolder() + cppFileName;
	FILE* output = nullptr;
	errno_t result = _wfopen_s(&output, fileFullPath.c_str(), L"wt");
	if (result != 0 || !output)
	{
	...
	fwprintf(output, L"}\n");
	fwprintf(output, L"\n");
	fwprintf(output, L"%s::%s(i32 maxMessageLength)\n", cn, cn);
	fwprintf(output, L"    :\n");
	fwprintf(output, L"    Font(msWidth, msHeight, (i8 const*)msTexels, msCharacterData,\n");
	fwprintf(output, L"        maxMessageLength)\n");
	fwprintf(output, L"{\n");
	fwprintf(output, L"}\n");
	fwprintf(output, L"\n");
	fwprintf(output, L"\n");
	fwprintf(output, L"i32 %s::msWidth = %d;\n", cn, width);
	fwprintf(output, L"i32 %s::msHeight = %d;\n", cn, height);
	fwprintf(output, L"\n");
	fwprintf(output, L"u8 %s::msTexels[%d] =\n", cn, numTexels);
	fwprintf(output, L"{\n");
	...
	fwprintf(output, L"};\n");
	fwprintf(output, L"\n");
	fwprintf(output, L"f32 %s::msCharacterData[257] =\n", cn);
	fwprintf(output, L"{\n");
	numPerRow = 8;
	...
	float characterData[257])
{
	...
	for (int i = 0; i < 256; ++i)
	{
		...
		TextOut(hDC, 8, 4 * height, msg, static_cast<int>(wcslen(msg)));
