...
ShaderNodeImplPtr HeightToNormalNodeHlsl::create()
{
    return std::make_shared<HeightToNormalNodeHlsl>();
}

void HeightToNormalNodeHlsl::computeSampleOffsetStrings(const string& sampleSizeName, const string& offsetTypeString, 
...
bool HeightToNormalNodeHlsl::acceptsInputType(const TypeDesc* type) const
{
    // Only support inputs which are float scalar
    return (type == Type::FLOAT && type->isScalar());
}

void HeightToNormalNodeHlsl::emitFunctionDefinition(const ShaderNode&, GenContext& context, ShaderStage& stage) const
{
...
void HeightToNormalNodeHlsl::emitFunctionCall(const ShaderNode& node, GenContext& context, ShaderStage& stage) const
{
    BEGIN_SHADER_STAGE(stage, Stage::PIXEL)
        const ShaderGenerator& shadergen = context.getShaderGenerator();
...
const string& HeightToNormalNodeHlsl::getTarget() const
{
