...
ShaderNodeImplPtr GeomPropValueNodeHlsl::create()
{
    return std::make_shared<GeomPropValueNodeHlsl>();
}

void GeomPropValueNodeHlsl::createVariables(const ShaderNode& node, GenContext&, Shader& shader) const
{
...
void GeomPropValueNodeHlsl::emitFunctionCall(const ShaderNode& node, GenContext& context, ShaderStage& stage) const
{
...
ShaderNodeImplPtr GeomPropValueNodeHlslAsUniform::create()
{
    return std::make_shared<GeomPropValueNodeHlslAsUniform>();
}

void GeomPropValueNodeHlslAsUniform::createVariables(const ShaderNode& node, GenContext&, Shader& shader) const
{
    const ShaderInput* geomPropInput = node.getInput(GEOMPROP);
    if (!geomPropInput || !geomPropInput->getValue())
    {
...
void GeomPropValueNodeHlslAsUniform::emitFunctionCall(const ShaderNode& node, GenContext& context, ShaderStage& stage) const
{
    BEGIN_SHADER_STAGE(stage, Stage::PIXEL)
